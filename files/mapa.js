...
function renderDetalle(spot) {
  ...
  // Galería de fotos
  // Si el spot no tiene fotos, muestra un placeholder automático
  ...
  contenedor.innerHTML = `
    <div class="detalle-header">
      <button class="detalle-cerrar" onclick="ocultarDetalle()" aria-label="Cerrar detalle">
  ...
  // Scroll suave hasta el detalle en móvil
  if (window.innerWidth <= 768) {
    setTimeout(() => contenedor.scrollIntoView({ behavior: "smooth", block: "start" }), 350);
  }
}
...
function ocultarDetalle() {
  ...
  if (marcadores.length > 0 && tipoActivo) {
    marcadores.forEach(({ spot, marker }) => {
